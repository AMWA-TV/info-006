...
Next retrieve the members of the root block by sending the following JSON formatted command to the NC-01 WebSocket. In the JSON command the value of `oid` 1 indicates we are directing this command at the [root block](https://specs.amwa.tv/ms-05-02/latest/docs/Blocks.html). The `methodId` with level 1 and index 1 is the [getter](https://specs.amwa.tv/ms-05-02/latest/docs/NcObject.html#generic-getter-and-setter) method and the `id` level and index of 2 and 2 respectively targets the `2p2` `members` property of the root [block](https://specs.amwa.tv/ms-05-02/latest/docs/Framework.html#ncblock).
...
The device responds with a JSON containing [NcBlockMemberDescriptor](https://specs.amwa.tv/ms-05-02/latest/docs/Framework.html#ncblockmemberdescriptor) member descriptors for the root block. Every member class can be determined by reading the `classId` field and they can be targeted with commands by using their `oid` value.
...
You will now make use of the generic [Get method](https://specs.amwa.tv/ms-05-02/latest/docs/NcObject.html#generic-getter-and-setter) (level 1, index 1) to read the value of the `userLabel` (level 1, index 6) property of the root block. All classes contain the `userLabel` property as defined in [NcObject](https://specs.amwa.tv/ms-05-02/latest/docs/Framework.html#ncobject).
...
The device responds to the above command with a JSON formatted response containing a result of type [NcMethodResultPropertyValue](https://specs.amwa.tv/ms-05-02/latest/docs/Framework.html#ncmethodresultpropertyvalue) holding the value of "Root".
...
Now we will set the `userLabel` (1p6) to "My mock device HOWTO" and verify the change has taken effect. We will use the generic [Set method](https://specs.amwa.tv/ms-05-02/latest/docs/NcObject.html#generic-getter-and-setter) for this (level 1, index 2). Copy and paste the following JSON formatted command to set the new value:
...
We can see that one of the members is the "Master gain" which is a non-standard control class because its `classId` property contains an authority key of 0 (see [NcClassId](https://specs.amwa.tv/ms-05-02/latest/docs/Framework.html#ncclassid) in the framework for more details). The `classId` property also informs us that the class is derived from the standard class [NcWorker](https://specs.amwa.tv/ms-05-02/latest/docs/Framework.html#ncworker).

One of the members of our root block discovered in the previous section was the [Class Manager](https://specs.amwa.tv/ms-05-02/latest/docs/Managers.html#class-manager):
...
The [Class Manager](https://specs.amwa.tv/ms-05-02/latest/docs/Managers.html#class-manager) allows us to retrieve a class descriptor for our "Master gain" control by sending the following command for `oid` 3 representing the Class Manager, invoking method `GetControlClass` (3m1) with arguments including the `classId` of our "Master gain".
...
Now we can attempt to retrieve the `overallStatus (level 3, index 1)` property of our receiver monitor (oid 10) by again making use of the generic [Get method](https://specs.amwa.tv/ms-05-02/latest/docs/NcObject.html#generic-getter-and-setter) (level 1, index 1).
